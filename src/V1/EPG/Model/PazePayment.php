...
class PazePayment implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets card
     *
     * @param Card|null $card Only the Contact billTo info can be provided alongside the token. This is optional, but it does provide a more complete response with the decrypted Google Pay payment info.
     *
     * @return self
     */
    ...
    {
        if (!is_null($payload_id) && (mb_strlen($payload_id) > 64)) {
    ...
    {
        if (!is_null($session_id) && (mb_strlen($session_id) > 255)) {
     ...
     *
     * @param VerificationResults|null $verification_results Verification results (all fields except cryptogramSecurity will be unprovided always).
     *
     * @return self
     */
