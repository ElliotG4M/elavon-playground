...
class Client
{
    ...
    public function createTransaction(
        ...
        string $merchantId, // Our Merchant ID
        string $merchantTransactionId, // Client's unique Transaction identifier (Generated by us)
        ...
        int $amount, // amount is in pence/cents - make sure to convert!
        ...
        string $redirectUrl, // redirect url once payment is completed - we'll need to link to an endpoint on our website where we can check the status of the transaction before redirecting to checkout success/failure
        string $systemId, // can be any old string for E_COMMERCE - probably just use the merchant ID again
        string $pushStatusUrl // used to receive status updates from Elavon. We'll probably
    ): AuthorizationResponse
    {
        // Including this payload in a test environment replaces the redirect URL in the response with a link to the payment emulator
        $payload = [
    ...
    ): GetTransactionStatusResponse
    {
        $request = new GetTransactionStatusRequest([
            'merchant_id' => $merchantId,
            'merchant_tx_id' => $transactionId
        ]);
        return $this->transactionStatusApi->getTransactionStatus($merchantId, $request, null, $transactionId);
    }
}